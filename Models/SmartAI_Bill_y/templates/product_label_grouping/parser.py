...
def parse_product_label_grouping(file_path: str) -> dict:
        ...
        for line in f:
            ...
            if in_promo_group:
                if key == 'SLTPRODGROUPLABEL':
                    ...
                    if current_promo_product:
                        current_promo_product['recurring_subtotal'] = \
                            to_float(value)

                elif key == 'SLTPRODLABEL_ONEOFF_SUBTOTAL':
                    if current_promo_product:
                        current_promo_product['oneoff_subtotal'] = \
                            to_float(value)
                continue
            ...
            # BPR12 subtotals
                ...
                if current_product:
                    current_product['recurring_subtotal'] = to_float(value)

            elif key == 'SLTPRODLABEL_ONEOFF_SUBTOTAL':
                if current_product:
                    current_product['oneoff_subtotal'] = to_float(value)
    ...
    # Telephone: first product label that is 10-11 digits
    for lbl in data['product_labels']:
        l = lbl['label']
        if l.isdigit() and 10 <= len(l) <= 11:
            data['telephone_number'] = l
            break
