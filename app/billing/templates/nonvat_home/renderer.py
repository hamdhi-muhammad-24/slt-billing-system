"""NonVAT Home Renderer (Sheet 19) - Total Charges + Payments + Usage sections independent."""
import os
from datetime import datetime
from core.pdf_renderer import BaseRenderer
from templates.nonvat_home.config import COORDS, CHARGES_TABLE, FONTS
...
class NonVATHomeRenderer(BaseRenderer):
    """Renders NonVAT Home invoice_of_summary (Sheet 19)."""
    ...
    def render(self, data):
        ...
        self._draw_usage_header_fixed(data)
        self._draw_usage_preview_rows_fixed(data)

        self._draw_usage_continuation(data)
    ...
    def _draw_badge(self, data):
        f = FONTS["badge"]
        self.text(*COORDS["badge_text"], "HOME", size=f["size"], bold=f["bold"])
    ...
    def _draw_taxes_only(self, data, y):
        """Taxes & Levies stay FLOWING with charges. Total Charges is independent now."""
        line_h = CHARGES_TABLE["line_h"]
        f = FONTS["taxes"]
    ...
    def _draw_payments_fixed(self, data):
        ...
        for p in data.get("payments", []):
            line = f"{p.get('date', '')} - {p.get('location', '')}"
    ...
    def _draw_usage_header_fixed(self, data):
        """3. Usage section header + subsection label - FIXED position."""
        ...
        header = f'Detailed Usage Charges for {first_section["label"]}'
        if first_section["phone"]:
            header += f' {first_section["phone"]}'
        page1_canvas.setFont("Helvetica-Bold", 8)
        page1_canvas.drawString(
            COORDS["usage_header_x"],
            COORDS["usage_header_y"],
            header
        )

        if first_sub["label"]:
            page1_canvas.drawString(
                COORDS["usage_subsection_x"],
                COORDS["usage_subsection_y"],
                first_sub["label"]
            )

    def _draw_usage_preview_rows_fixed(self, data):
        """4. Usage preview rows - FIXED position (max N rows)."""
        sections = [s for s in data.get("usage_sections", []) if s["subsections"]]
        if not sections:
            return
        ...
        y = COORDS["usage_rows_start_y"]
        col_x = COORDS["usage_col_x"]
        amt_x = COORDS["usage_amount_x"]
        row_limit = COORDS["usage_row_limit"]
        line_h = COORDS["usage_row_line_h"]

        headers = first_sub["headers"]
        combine = len(headers) >= 2 and headers[0] == 'Date' and headers[1] == 'Time'

        page1_canvas.setFont("Helvetica", 8)
        for row in first_sub["rows"][:row_limit]:
            display_row = [f"{row[0]}  {row[1]}"] + row[2:] if combine else row
            for i, val in enumerate(display_row[:len(col_x) + 1]):
                col_x_val = col_x[i] if i < len(col_x) else amt_x
                if i == len(display_row) - 1:
                    page1_canvas.drawRightString(col_x_val, y, val)
                else:
                    page1_canvas.drawString(col_x_val, y, val)
            y -= line_h



    def _draw_usage_continuation(self, data):
        """Full usage details on page 2+ — narrow box, fixed alignment + totals."""
        sections = [s for s in data.get("usage_sections", []) if s["subsections"]]
        if not sections:
            return
        ...
        first_sub = first_section["subsections"][0]
        row_limit = COORDS["usage_row_limit"]
        remaining_first_rows = first_sub["rows"][row_limit:]
        ...
        col_x = [50, 130, 190]  # Date&Time | Dialled | Duration
        amount_x = 300
        box_right = 305
        total_label_x = 45
        ...
        def get_last_numeric_value(row):
            """Reliably extract the numeric charge from a row.
            Handles trailing empty strings and non-numeric junk."""
        ...
        def sum_rows_reliable(rows):
            """Sum charges from all rows (finds last numeric value in each row)."""
        ...
        def draw_table(sub, rows, print_header_line, show_section_header,
            ...
            if print_header_line and sub["label"]:
                ...
                y_state["y"] -= line_h * 1.2  # ← MORE gap before box so no overlap
            ...
            if print_header_line:
                # Draw narrow box — height matches the row, no overflow
                box_y = y_state["y"] - 2
                box_h = line_h + 2
                try:
                ...
                # Draw header text INSIDE the box
            ...
            # Data rows — properly align charge to amount_x
            for row in rows:
                if y_state["y"] < y_min:
                    self.new_page()
                    y_state["y"] = 780
                ...
                # Extract the numeric charge (guaranteed last numeric value)
                charge_val = get_last_numeric_value(row)

                # Draw non-charge columns (all except the last)
                for i in range(min(len(disp) - 1, len(col_x))):
                    x = col_x[i]
                    self.text(x, y_state["y"], str(disp[i]), size=font_row)

                # Draw charge — ALWAYS right-aligned to amount_x
        ...
        first_sub_total = sum_rows_reliable(first_sub["rows"])

        if remaining_first_rows:
            draw_table(first_sub, remaining_first_rows,
                       print_header_line=False, show_section_header=False,
                       section_label=first_section["label"],
                       section_phone=first_section["phone"])

        self.text(total_label_x, y_state["y"], f'Total for {first_sub["label"]}',
                  size=font_subtotal, bold=True)
        self.number(amount_x, y_state["y"], first_sub_total, decimals=3,
        ...
        for sub in first_section["subsections"][1:]:
            ...
            draw_table(sub, sub["rows"], print_header_line=True,
        ...
        # First section grand total
        section_grand_total = first_section.get("grand_total", 0)
        if not section_grand_total:
            section_grand_total = sum(sum_rows_reliable(s["rows"])
                                      for s in first_section["subsections"])

        self.text(total_label_x, y_state["y"],
                  f'Total Usage Charges for {first_section["label"]}',
        ...
        for section in sections[1:]:
            for sub in section["subsections"]:
                sub_total = sum_rows_reliable(sub["rows"])
                draw_table(sub, sub["rows"], print_header_line=True,
                           show_section_header=(sub is section["subsections"][0]),
